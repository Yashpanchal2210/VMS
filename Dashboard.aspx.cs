        ...
        private void LoadOfficerSheet()
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    SqlDataAdapter da = new SqlDataAdapter("SELECT i.ItemName AS ItemName, i.RationScaleOfficer, a.AltItemName AS AlternateItemName FROM Items i LEFT JOIN AlternateItem a ON i.ItemID = a.ItemID", conn);
                    DataTable dt = new DataTable();
                    da.Fill(dt);
        ...
        protected void ExportOfficersButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (ExcelPackage excelPackage = new ExcelPackage())
                {
                    ...
                    // Assuming you have a GridView called GridViewOfficersSheet
                    DataTable gridViewData = new DataTable();
                    gridViewData.Columns.Add("Ser");
                    gridViewData.Columns.Add("Item");
                    gridViewData.Columns.Add("Strength");
                    gridViewData.Columns.Add("Scale");
                    gridViewData.Columns.Add("QtyEntitled");
                    gridViewData.Columns.Add("QtyIssued");
                    gridViewData.Columns.Add("AlternateItemName");

                    // Assuming GridViewOfficersSheet has the necessary columns and rows
                    foreach (GridViewRow row in GridViewOfficersSheet.Rows)
                    {
                        DataRow dataRow = gridViewData.NewRow();
                        dataRow["Ser"] = row.Cells[0].Text;
                        dataRow["Item"] = row.Cells[1].Text;
                        dataRow["Strength"] = row.Cells[2].Text;
                        dataRow["Scale"] = row.Cells[3].Text;
                        dataRow["QtyEntitled"] = row.Cells[4].Text;
                        dataRow["QtyIssued"] = row.Cells[5].Text;
                        dataRow["AlternateItemName"] = row.Cells[6].Text;
                    ...
                    var groupedData = gridViewData.AsEnumerable()
                        ...
                        {
                            ItemName = group.Key,
                            Strength = group.Sum(row => Convert.ToInt32(row.Field<string>("Strength"))),
                            Scale = group.Average(row => Convert.ToDecimal(row.Field<string>("Scale"))),
                            QtyEntitled = group.Sum(row => Convert.ToDecimal(row.Field<string>("QtyEntitled"))),
                            QtyIssued = group.Sum(row => Convert.ToDecimal(row.Field<string>("QtyIssued"))),
                            AlternateItems = group.Select(row => row.Field<string>("AlternateItemName")).Distinct().ToList()
                        });
                    ...
                    foreach (var group in groupedData)
                    {
                        ...
                        if (currentRow < 1 || currentRow > worksheet.Dimension.End.Row || 1 < 1 || 1 > worksheet.Dimension.End.Column)
                        {
                            throw new ArgumentOutOfRangeException("Index out of range: " + currentRow);
                        }

                        worksheet.Cells[currentRow, 1].Value = serNumber++;
                        worksheet.Cells[currentRow, 2].Value = group.ItemName;
                        worksheet.Cells[currentRow, 3].Value = group.Strength;
                        worksheet.Cells[currentRow, 4].Value = "x";
                        worksheet.Cells[currentRow, 5].Value = group.Scale;
                        ...
                        {
                            // Ensure indices are within valid ranges
                            if (currentRow < 1 || currentRow > worksheet.Dimension.End.Row || 2 < 1 || 8 > worksheet.Dimension.End.Column)
                            {
                                throw new ArgumentOutOfRangeException("Index out of range: " + currentRow);
                            }
